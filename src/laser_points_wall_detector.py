...
class LaserPointsWallDetector(WallDetector):
    ...
    def detect_wall(self):
        ...
        max_wall_deg = np.min([np.max([minp1_deg,minp2_deg]) +  self.angel_offset, 180])# maximum angle to look for a wall, at most 180 (max angel of lidar)
        ...
        while wall_end <= max_wall_deg*4-1: 
            found_wall,data = self.__find_wall_in_range(wall_start,wall_end,laser_msg)
            if found_wall:
                return data
            else:
                wall_start = data
    ...
    def __find_wall_in_range(self,wall_start,wall_end,laser_msg):
        ...
        if points_not_on_poly < self.points_allowed_not_on_poly:
                return True,(m,b)                           # return poly of the wall
