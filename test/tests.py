...
class Test(TestCase):
    ...
    def test_polar_to_cartesian_first_quadrant(self):
        ...
        for idx, inp in enumerate(inputs):
            ...
            self.assertTrue(np.isclose(
                p_out[0], p_expected[0], 0, err_threash))
            self.assertTrue(np.isclose(
                p_out[1], p_expected[1], 0, err_threash))
    ...
    def test_find_two_closest_points_main_test_case(self):
        ...
        laser[424] = 1
        laser[456] = 2
        minp1, minp1_deg, minp2, minp2_deg = tracker.find_two_closest_points(
            laser)
        self.assertAlmostEqual(minp1, 1, 5, 'minp1: '+str(minp1), 0.001)
        self.assertAlmostEqual(minp2, 2, 5, 'minp2: '+str(minp2), 0.001)
        self.assertAlmostEqual(minp1_deg, 424/4, 5,
                               'minp1_deg: '+str(minp1_deg), 0.001)
        self.assertAlmostEqual(minp2_deg, 456/4, 5,
                               'minp2_deg: '+str(minp2_deg), 0.001)
    ...
    def test_find_orthogonal_line_through_point(self):
        ...
        for idx, slope_and_point in enumerate(slopes_and_points):
            ...
            self.assertEqual(
                polys[idx], tracker.find_orthogonal_line_through_point(m, x, y))
